...
// SE (root é NULO) ENTÃO
// FAÇA root IGUAL A NOVO NÓ (key, data)
// SENÃO SE (key > root->key) ENTÃO
// root->right =
...
// Busca, ver material.
// SE (root é NULO) ENTÃO RETORNE NULO
// SE (key == root->key) ENTÃO RETORNE root
// SE (key > root->key) ENTÃO
...
void BinaryTree::show(Node * root) {
	// Exiber a árvore em ordem (in-order): esquerda, raiz, direita
	// Deve exibir o valor da chave a a altura por nó da árvore
	// Formato: (<valor>, <altura>), separado por espaços, exemplo:
	// (1, 0) (3, 2) (2, 0) ... (10, 3)
	if (root == NULL) return;
